...
func TestFFVotingOpensOnlyWhenEveryAnswerIsIn(t *testing.T) {
	...
	expected := fakefiller.AnswersFor(fakefiller.GameModeFacts, len(game.players), fakefiller.DefaultAnswersPerPlayer)
	written := 0
...
func TestFFAnswerProgressCarriesCountsAndNoContent(t *testing.T) {
	...
	if body.AnswersIn != 1 || body.AnswersNeeded != fakefiller.AnswersFor(fakefiller.GameModeFacts, len(game.players), fakefiller.DefaultAnswersPerPlayer) {
		t.Errorf("counts = %d/%d, want 1/%d", body.AnswersIn, body.AnswersNeeded, fakefiller.AnswersFor(fakefiller.GameModeFacts, len(game.players), fakefiller.DefaultAnswersPerPlayer))
	}
