...
func (s *GormStore) CreateLobby(ctx context.Context, lobby *WWLobby) error {
...
func (s *GormStore) LobbyByCode(ctx context.Context, code string) (*WWLobby, error) {
	var lobby WWLobby
...
func (s *GormStore) WaitingLobbyByOwnerID(ctx context.Context, userID string) (*WWLobby, error) {
	var lobby WWLobby
...
func (s *GormStore) AddLobbyPlayer(ctx context.Context, player *WWLobbyPlayer) error {
...
func (s *GormStore) DeleteLobby(ctx context.Context, code string) error {
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("lobby_id = ?", code).Delete(&WWLobbyPlayer{}).Error; err != nil {
			return fmt.Errorf("delete lobby players: %w", err)
		}
		if err := tx.Where("id = ?", code).Delete(&WWLobby{}).Error; err != nil {
...
func (s *GormStore) DeleteLobbiesOlderThan(ctx context.Context, before time.Time) (int64, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("lobby_id IN ?", codes).Delete(&WWLobbyPlayer{}).Error; err != nil {
			return fmt.Errorf("delete lobby players: %w", err)
		}
		result := tx.Where("id IN ?", codes).Delete(&WWLobby{})
		if result.Error != nil {
...
func (s *GormStore) StartLobby(ctx context.Context, lobby *WWLobby, game *WWMultiDeviceGame) error {
...
func (s *GormStore) GameByID(ctx context.Context, id uuid.UUID) (*WWMultiDeviceGame, error) {
	var game WWMultiDeviceGame
...
func (s *GormStore) GamesByUserID(ctx context.Context, userID string) ([]*WWMultiDeviceGame, error) {
	var games []*WWMultiDeviceGame

	err := s.db.WithContext(ctx).
		Joins("JOIN ww_game_players ON ww_game_players.game_id = ww_games.id").
		Where("ww_game_players.user_id = ? AND ww_games.status = ?", userID, GameInProgress).
		Order("ww_games.created_at DESC").
		Find(&games).Error
	if err != nil {
...
func (s *GormStore) DeleteGamesOlderThan(ctx context.Context, before time.Time) (int64, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		err := tx.Model(&WWMultiDeviceGame{}).
			Where("created_at < ?", before).
		...
		if len(roundIDs) > 0 {
			if err := tx.Where("round_id IN ?", roundIDs).Delete(&WWVote{}).Error; err != nil {
				return fmt.Errorf("delete votes: %w", err)
			}
			if err := tx.Where("round_id IN ?", roundIDs).Delete(&WWOption{}).Error; err != nil {
				return fmt.Errorf("delete options: %w", err)
			}
			if err := tx.Where("id IN ?", roundIDs).Delete(&WWRound{}).Error; err != nil {
				return fmt.Errorf("delete rounds: %w", err)
			}
		}
		if err := tx.Where("game_id IN ?", gameIDs).Delete(&WWGamePlayer{}).Error; err != nil {
			return fmt.Errorf("delete game players: %w", err)
		}
		result := tx.Where("id IN ?", gameIDs).Delete(&WWMultiDeviceGame{})
		if result.Error != nil {
...
// SaveAnswers writes one player's whole batch and reports how many answers the game holds afterwards.
func (s *GormStore) SaveAnswers(ctx context.Context, gameID uuid.UUID, options []WWOption) (int, error) {
	var answered int64
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		err := tx.Model(&WWOption{}).
			Joins("JOIN ww_rounds ON ww_rounds.id = ww_round_options.round_id").
			Where("ww_rounds.game_id = ?", gameID).
...
// OpenVoting flips a game into its second half and writes down the order the answers are to be shown in.
func (s *GormStore) OpenVoting(ctx context.Context, gameID uuid.UUID, slots []SlotAssignment) (bool, error) {
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		res := tx.Model(&WWMultiDeviceGame{}).
...
// RecordVote writes one vote, pays for it, and parks the round on its reveal if it was the last vote it was waiting for.
func (s *GormStore) RecordVote(ctx context.Context, in RecordVoteInput) (*RecordVoteResult, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		for _, authorID := range in.AuthorIDs {
			if err := addScore(tx, in.GameID, authorID, VotePoints); err != nil {
				return err
			}
		}

		var round WWRound
		err := tx.Preload("Options").Preload("Votes").Where("id = ?", in.Vote.RoundID).First(&round).Error
		if err != nil {
			return fmt.Errorf("select round: %w", err)
		}
		result.Votes = len(round.Votes)

		if result.Votes < in.VotersNeeded {
			return nil
		}
		...
		// Read off the votes as they stand inside this transaction, so a vote racing this one cannot be missed.
		if sweep := SweepSlot(round); sweep >= 0 {
			for _, option := range round.OptionsInSlot(sweep) {
				if err := addScore(tx, in.GameID, option.AuthorID, SweepBonus); err != nil {
					return err
				}
			}
		}
...
func (s *GormStore) AdvanceRound(ctx context.Context, in AdvanceRoundInput) (bool, error) {
	...
	res := s.db.WithContext(ctx).
		...
		Updates(update)
	if res.Error != nil {
		return false, fmt.Errorf("advance round: %w", res.Error)
	}
	return res.RowsAffected == 1, nil
}

func addScore(tx *gorm.DB, gameID uuid.UUID, userID string, points int) error {
	err := tx.Model(&WWGamePlayer{}).
