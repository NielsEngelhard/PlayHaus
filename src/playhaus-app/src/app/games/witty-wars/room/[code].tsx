...
import PlayingGame from "@/features/witty-wars/components/play/PlayingGame";
import { useGame } from "@/features/witty-wars/useGame";
import { useLobby } from "@/features/witty-wars/useLobby";
...
export default function WittyWarsRoomPage() {
    ...
    useEffect(() => {
        ...
        router.replace(ROUTES.wittyWarsRoom(rematchCode) as RelativePathString);
        ...
        return (
            <RoomClosedNotice
                message={gameId !== undefined
                    ? t('wittyWars.lobby.hostStoppedGame')
                    : t('wittyWars.lobby.hostClosedLobby')}
...
function RoomGame({ table, onClose, onFinish, leaving, onStay, onLeave }: RoomGameProps) {
    ...
    return (
        <View style={styles.page}>
            ...
            <PopupModal
                visible={leaving}
                title={t('wittyWars.lobby.confirmLeave.title')}
                message={t('wittyWars.lobby.confirmLeave.message')}
                tone='danger'
                onRequestClose={onStay}
                actions={<>
                    <TextButton
                        text={t('wittyWars.lobby.confirmLeave.action')}
                        variant='primary'
                        fullWidth
                        onPress={onLeave}
                    />
...
function RoomResults({ state, table }: RoomResultsProps) {
    ...
    if (game === null) {
        return <LoadingPage message={t('wittyWars.results.loading')} />;
    }
    ...
    return (
        <ScoreBoardScreen
            ...
            youId={user?.id}
            onClose={() => router.replace(ROUTES.wittyWarsIndex)}
            action={state.isHost
                ? {
                    text: state.rematching ? t('wittyWars.lobby.opening') : t('scoreboard.playAgain'),
                    icon: 'rotate-ccw',
