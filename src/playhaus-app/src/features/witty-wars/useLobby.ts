import {
    ...
    rematchWWLobby,
    startWWLobby,
    updateWWLobbySettings,
    type WWGameMode,
    type WWLobby,
    type WWLobbySettings
} from '@/api/calls/witty-wars-lobby';
...
// The Witty Wars room, from opening it to being carried out of it into the next one.

// Kept under its old name for the one page that waits on it.
export const settleWWGiveBacks = settleGiveBacks;

export interface WWLobbyState {
    lobby: WWLobby | null
    ...
    reload: () => void
    // Host only.
    updateSettings: (settings: WWLobbySettings) => void
}

export function useLobby(code?: string, gameMode?: WWGameMode): WWLobbyState {
    const { user, status } = useAuth();
    const [lobby, setLobby] = useState<WWLobby | null>(null);
    ...
    const letGo = useCallback((giveBack: { host: boolean } | null) => {
        ...
        track(giveBack.host ? deleteWWLobby(heldCode) : leaveWWLobby(heldCode));
    }, []);
    ...
    const load = useCallback(async () => {
        ...
        try {
            const opened = code === undefined
                ? await createWWLobby(locale.current, gameMode)
                : await joinWWLobby(code);
            ...
            if (!mounted.current) {
                ...
                letGo(opened.status === 'waiting' ? { host: isHostOfWW(opened, userId) } : null);
                return;
            }

            // A room that has already started is a game, not a room.
            owed.current = opened.status === 'waiting'
                ? { code: opened.code, host: isHostOfWW(opened, userId) }
                : null;
            ...
            setError(wwLobbyErrorMessage(failure));
        }
    ...
    const { status: connection, online } = useRoomSocket<WWServerEvent>({
        room: openCode === undefined ? undefined : wwRoom(openCode),
        enabled: signedIn,
        onEvent
    });
    ...
    const isHost = lobby !== null && isHostOfWW(lobby, userId);

    const start = useCallback(async (): Promise<WWLobby | null> => {
        ...
        try {
            ...
            const started = await startWWLobby(lobby.code);
            ...
            if (mounted.current) setActionError(wwLobbyErrorMessage(failure));
    ...
    const rematch = useCallback(async () => {
        ...
        try {
            const next = await rematchWWLobby(lobby.code);
            if (!mounted.current) return;
    ...
    const updateSettings = useCallback(async (next: WWLobbySettings) => {
        ...
        try {
            const saved = await updateWWLobbySettings(lobbyCode, next);
            if (!mounted.current) return;
            ...
            setActionError(wwLobbyErrorMessage(failure));
        } finally {
            writing.current = false;
    ...
    const close = useCallback(async () => {
        ...
        try {
            await (leaving.host ? deleteWWLobby(leaving.code) : leaveWWLobby(leaving.code));
        } catch {
            // Best effort.
        } finally {
