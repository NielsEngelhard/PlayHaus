...
interface Props {
    /** The screen that opens a room, held back until it is certain there is not already one. */
    children: ReactNode
}

// The room this player is already on the hook for is theirs to resume or throw away before another can be opened.
export default function CurrentRoomGuard({ children }: Props) {
    ...
    function resume(lobby: Lobby) {
        const href = lobby.kind === 'tournament'
            ? ROUTES.leagueOfLettersTournamentRoom(lobby.code)
            : ROUTES.leagueOfLettersRoom(lobby.code);
