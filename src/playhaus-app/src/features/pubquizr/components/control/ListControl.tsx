...
export default function ListControl({ busy, emit, error, onSettle, round, turn }: Props) {
    ...
    // Emits nothing: a frame from render is a side effect, and the screen reads nothing for a question it is not on.
    ...
    // The one writer of the awards, so no mutation can reach the screen without a frame -- and never from a `setAwards` updater, which must stay pure.
    function apply(next: ListAwards) {
        setAwards(next);

        const ids = turn.answers
            .filter(answer => (next[answer.id] ?? null) !== null)
            .map(answer => answer.id);

        // The ids, because round 5's answers belong on the screen as they are credited.
        emit({ kind: 'awards', questionId, awarded: ids.length, ids });
    }

    /** Credit an answer to one seat, or to nobody. */
    function credit(answerId: string, seat: number | null) {
        apply({ ...awards, [answerId]: seat });
    }

    function toggleInTime(answerId: string) {
        const next: ListAwards = {
            ...awards,
            [answerId]: (awards[answerId] ?? null) === null ? turn.guesser.seat : null
        };

        apply(next);

        // Outside the updater, which `ListBoard` can get away with and a frame-emitting one cannot.
    ...
    function startTimer() {
        setStage('running');
        emit({ kind: 'timer', questionId, endsAt: Date.now() + LIST_SECONDS * 1000 });
    }
