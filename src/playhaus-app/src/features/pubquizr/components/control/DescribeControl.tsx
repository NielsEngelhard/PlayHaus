...
export default function DescribeControl({ busy, emit, error, onSettle, round, turn }: Props) {
    ...
    // Reset during render, like every other board here. It emits nothing, because a frame from render is a side effect -- and the screen reads nothing for a question it is not on.
    ...
    const questionId = turn.words[0].dealt.id;
    ...
    function apply(next: DescribeAwards) {
        setAwards(next);

        const got = turn.words.filter(word => (next[word.dealt.id] ?? null) !== null).length;
    ...
    function toggleInTime(wordId: string) {
        apply({
            ...awards,
            [wordId]: (awards[wordId] ?? null) === null ? turn.guesser.seat : null
        });
    }
