...
import { ROUND_CLOSEST } from '@/features/pubquizr/round-three';
import { seatAt, seatsOf, type Seat } from '@/features/pubquizr/seats';

// Who runs the turn right now: nobody in rounds 2 and 3, and whoever holds the words in round 4.
export function currentQuizmasterOf(session: QuizSession): Seat | null {
    if (session.currentRound === ROUND_CHOICE || session.currentRound === ROUND_CLOSEST) return null;
