import {
    ...
    rematchOOULobby,
    startOOULobby,
    updateOOULobbySettings,
    type OOULobby,
    type OOULobbySettings
} from '@/api/calls/one-of-us-lobby';
...
// The multi-device One of Us room, from opening it to being carried out of it into the next one.

// The page that opens a room waits on this before asking the server what it still owns.
export const settleOOUGiveBacks = settleGiveBacks;

export interface OOULobbyState {
    lobby: OOULobby | null
    ...
    reload: () => void
    // Host only.
    updateSettings: (settings: OOULobbySettings) => void
}

export function useOneOfUsLobby(code?: string): OOULobbyState {
    const { user, status } = useAuth();
    const [lobby, setLobby] = useState<OOULobby | null>(null);
    ...
    const letGo = useCallback((giveBack: { host: boolean } | null) => {
        ...
        track(giveBack.host ? deleteOOULobby(heldCode) : leaveOOULobby(heldCode));
    }, []);
    ...
    const load = useCallback(async () => {
        ...
        try {
            const opened = code === undefined
                ? await createOOULobby(locale.current)
                : await joinOOULobby(code);
            ...
            if (!mounted.current) {
                ...
                letGo(opened.status === 'waiting' ? { host: isHostOfOOU(opened, userId) } : null);
                return;
            }

            // A room that has already started is a game, not a room.
            owed.current = opened.status === 'waiting'
                ? { code: opened.code, host: isHostOfOOU(opened, userId) }
                : null;
            ...
            setError(oneOfUsLobbyErrorMessage(failure));
        }
    ...
    const onEvent = useCallback((event: OOUServerEvent) => {
        ...
        switch (event.type) {
            ...
            default:
                // Everything else on this socket belongs to the board, which reads the same room through its own hook.
                return;
        }
    }, []);
    ...
    const { status: connection, online } = useRoomSocket<OOUServerEvent>({
        room: openCode === undefined ? undefined : oouRoom(openCode),
        enabled: signedIn,
        onEvent
    });
    ...
    const isHost = lobby !== null && isHostOfOOU(lobby, userId);

    const start = useCallback(async (): Promise<OOULobby | null> => {
        ...
        try {
            ...
            const started = await startOOULobby(lobby.code);
            ...
            if (mounted.current) setActionError(oneOfUsLobbyErrorMessage(failure));
    ...
    const rematch = useCallback(async () => {
        ...
        try {
            const next = await rematchOOULobby(lobby.code);
            if (!mounted.current) return;
    ...
    const updateSettings = useCallback(async (next: OOULobbySettings) => {
        ...
        try {
            const saved = await updateOOULobbySettings(lobbyCode, next);
            if (!mounted.current) return;
            ...
            setActionError(oneOfUsLobbyErrorMessage(failure));
        } finally {
            writing.current = false;
    ...
    const close = useCallback(async () => {
        ...
        try {
            await (leaving.host ? deleteOOULobby(leaving.code) : leaveOOULobby(leaving.code));
        } catch {
            // Best effort.
        } finally {
